...
class DynMajorArticle:
    ...
    async def run(self, dyn_maojor: Major, dyn_type) -> Optional[Image.Image]:
        try:
            ...
            self.make_title(dyn_maojor.article.title),self.make_desc(dyn_maojor.article.desc,dyn_maojor.article.label)]
    ...
    async def make_desc(self,desc,lable):
        ...
        for i in desc:
            ...
            if x_position >= 990:
                ...
                if y >= y_constrain:
                    self.draw.text((x_temp,y-int(next_offset[3] * 1.3)),"...",fill=self.dyn_color.dyn_silver_gray,font=font)
                    break
